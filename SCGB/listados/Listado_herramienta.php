...
document.write("<span class='class8' id=fecha>"+dayarray[day]+" "+daym+" de "+montharray[month]+" de "+year+"</span>")
	...
	$sql = "SELECT p.CODIGOPRODUCTO,p.P_NOMBRE,p.P_MARCA,p.P_MODELO,h.H_TIPOHERRAMIENTA,s.S_CANTIDAD FROM  HERRAMIENTA h,PRODUCTO p,STOCK s  WHERE p.CODIGOPRODUCTO=h.CODIGOPRODUCTO and p.CODIGOPRODUCTO=s.CODIGOPRODUCTO and p.P_ESTADO=1 ";
	conectar();
	$rs=mysql_query($sql,$conexion);	
	while ($row=mysql_fetch_array($rs)){
